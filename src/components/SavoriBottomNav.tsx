        ...
        <div className="fixed bottom-0 left-0 w-full z-50 pointer-events-none">
            ...
            <div className="flex justify-center pb-6 pt-2 px-4 max-w-md mx-auto relative pointer-events-auto">
                <nav className="bg-[#161b22]/90 backdrop-blur-md border border-white/10 rounded-3xl p-1.5 flex justify-between items-center w-full shadow-2xl">
                    ...
                    {/* Dashboard / Scan */}
                    <Link href="/dashboard" className="flex flex-col items-center justify-center w-full h-14 rounded-2xl hover:bg-white/5 text-slate-400 hover:text-white transition group">
                    ...
                    {/* Budget */}
                    <Link href="/budgets" className={`flex flex-col items-center justify-center w-full h-14 rounded-2xl transition group ${isActive('/budgets') ? 'bg-white/10 text-white' : 'hover:bg-white/5 text-slate-400 hover:text-white'}`}>
                        {isActive('/budgets') && (
                            <motion.span
                                layoutId="nav-dot"
                    ...
                    {/* Transactions */}
                    <Link href="/expenses" className="flex flex-col items-center justify-center w-full h-14 rounded-2xl hover:bg-white/5 text-slate-400 hover:text-white transition group">
                        <Shield className={`w-6 h-6 mb-0.5 ${isActive('/expenses') ? 'text-orange-400' : ''}`} />
                        <span className="text-[10px] font-medium">Transakcje</span>
                    </Link>

                    {/* Menu / Sidebar */}
                    <Link href="/settings" className="flex flex-col items-center justify-center w-full h-14 rounded-2xl bg-violet-500 text-white shadow-lg shadow-violet-500/20 ml-1 active:scale-95 transition-transform">
